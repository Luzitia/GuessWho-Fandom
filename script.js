...
// Wir prüfen exakt, ob die Container-IDs auf der aktuellen Seite existieren
const menuContainer = document.getElementById("menu-container");
const gameContainer = document.getElementById("game-container");

// ==========================================
// MENÜ-LOGIK (Wird nur auf index.html ausgeführt)
// ==========================================
if (menuContainer) {
    ...
    // Sicherstellen, dass die Elemente wirklich da sind, bevor wir Listener hinzufügen
    if (createRoomBtn && joinRoomBtn) {
        ...
        createRoomBtn.addEventListener("click", () => {
            ...
            }).catch(err => alert("Firebase Fehler: " + err.message));
        });
...
// ==========================================
// SPIEL-LOGIK (Wird nur auf game.html ausgeführt)
// ==========================================
if (gameContainer) {
    ...
    // Falls jemand die Seite ohne Room-ID lädt, zurück zur Lobby
    if (!roomId) {
        ...
        if (playerTitle) playerTitle.innerText = (myRole === "p1") ? "Spieler 1 (Host)" : "Spieler 2 (Gast)";
        if (roomDisplay) roomDisplay.innerText = `Raum-Code: ${roomId}`;
        ...
        onValue(gameRef, (snapshot) => {
            ...
            if (!state) {
                if (statusBanner) statusBanner.innerText = "Fehler: Raumdaten verloren gegangen.";
                return;
            }
            renderGame(state);
        });
        ...
        function renderGame(state) {
            ...
            if (opponentCounter) opponentCounter.innerText = `Gegner hat noch ${oppOpenCards}/${BOARD_SIZE} Karten offen`;
            ...
            if (statusBanner && globalGuessBtn) {
        ...
        if (globalGuessBtn) {
            globalGuessBtn.addEventListener("click", () => {
                ...
                if (guessModeActive) {
                    ...
                    if (boardContainer) boardContainer.classList.add("guess-mode-active");
                    if (statusBanner) {
                    ...
                    globalGuessBtn.classList.remove("active");
                    if (boardContainer) boardContainer.classList.remove("guess-mode-active");
                    if (statusBanner) {
        ...
        function executeFinalGuess(guessedId, state) {
            ...
            guessModeActive = false;
            if (globalGuessBtn) globalGuessBtn.classList.remove("active");
            if (boardContainer) boardContainer.classList.remove("guess-mode-active");
            update(gameRef, updates);
        }
    }
